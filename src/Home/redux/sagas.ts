...
import { clearLoginDetailsAction, fetchRemoteConfigAction, forgotPasswordAction, loginAction, logoutAction, registerAction, reloadUserAction, sendMessageAction, storeLoginDetailsAction, storeRemoteConfigAction, storeUsersListAction, onlineAction, storeOnlineAction } from "./actions";
...
export function* loginSaga(action: { payload: { email: string, password: string } }): any {
  ...
  try {
    ...
    if (Object.keys(usersList).length < 1) {
      ...
      yield call(Firestore.updateUser, { id, details: { ...details, name: name ?? details?.name, photo: photo ?? details.photo, team } })
    }
